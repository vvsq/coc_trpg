...
| `GET /llm/status` | LLM 配置状态（key 掩码）+ 预设 + **全局** token 总账 |
| `GET /rooms/{id}/usage` | **本房间**（一场次）token 消耗，仅 KP（query `kp_name`，2026-09-10） |
| `POST /rooms/{id}/card-review` | AI 检卡（2026-09-10）：逐卡列出不合理处与改法，仅建议不拦截；同步调轻任务模型（前端超时放宽到 180s） |
...
**`skills.py`** —— 技能基础值解析（2026-09-10，用户反馈 #4）
- `base_expr_value(expr, attrs)`：把技能表的 `base_expr` 解算成数值（`闪避 = DEX/2` 向下取整、`母语 = EDU`），纯函数不查库。
- 用途：检定下放时"卡上没加点的技能"按基础值结算（任何技能都能尝试）；`occupation.py::SKILL_MAX_AT_CREATION = 90` 是单技能创建上限（**规则书未载明**：通读第三章全节确认，取 KP 裁定值，可改常量回退 80）。
...
- 真实响应经 `usage.record_usage()` 记账（含**命中前缀缓存的 token**：`_cached_tokens()` 读 `usage.prompt_tokens_details.cached_tokens`）；`ping`（attempts=1）与 `list_models`（10s 超时）。
...
- 字段：`calls / prompt_tokens / completion_tokens / cached_tokens`；读取时附 `cache_hit_rate = cached/prompt`。
- **`prompt_tokens` 只是名义输入**：DashScope 隐式前缀缓存自动生效（同前缀第二次实测命中 ~99%，最小可缓存前缀 1024 tokens，按块对齐），命中部分的 `cached_tokens` 按折扣计价——面板据此显示「命中 N（命中率 X%）· 实际计费输入 = 名义 − 命中」。
- `cached_tokens` 是 `prompt_tokens` 的子集，落库前夹取（供应商上报越界值不污染账面）；无缓存能力的供应商恒为 0。
...
- **可见性标注（2026-09-10 修复）**：`_session_line()` 统一渲染【近期剧情】行（协同/全自动共用，防两处漂移）——`secret=True` 的消息（keeper 笔记 / 暗骰结果，与公开叙事同属 narrative 频道）打上「·仅KP」标记，段头写明"玩家看不到、禁止写进公开内容"。原因：不标注时模型分不清玩家看过没有，而 D8 的守秘过滤是**逐字替换**，兜不住被改写过的守秘内容。
- **`latest_action` 只取玩家可见消息**：`_collect_context`（keeper / suggest 同源）用 `next((m for m in recent if not m.secret), '')`——全自动每轮结尾都会落一条 keeper 笔记，取 `recent[0]` 会把自己的守秘笔记当成玩家行动喂回去（实测复现）。
...
**`card_review.py`** —— AI 检卡（2026-09-10，用户反馈 #5）
- `_card_digest(data)`：整卡压成可读摘要（属性/衍生/技能值降序前 25/背景八要素/随身物品/武器）。
- `module_review_digest(session, room_id)`（在 module_context.py）：只取模组**公开层**（标题/基调/背景/钩子/分幕标题/NPC 公开身份/时钟名），不注入守秘字段。
- `build_review_messages` → 轻任务模型（json_mode）→ `parse_review`（剥围栏/剥尾逗号；`overall` 与 `issues` 强制对齐；过滤幻觉玩家名）。
- 口径：**只给建议不拦截开团**（用户决策）；无已绑卡玩家时不发起 LLM 调用。
...
| `agent.ts` | agent-mode/建议生成/LLM status·config·models·test（超时 90s）/kp-styles/房间切风格/本场消耗（`getRoomUsage`）/检卡（`reviewRoomCards`，超时 180s） |
...
| `LlmSettingsDialog.vue` | LLM 设置弹窗：预设 chips、主/轻模型、探测/测试、Token 总账（含**前缀缓存命中率与实际计费输入**）、mock 提示。 |
| `ModuleUploadDialog.vue` | 模组上传（拖拽/点选 → multipart → 提取字数反馈），不触发解析。 |
| `ModuleParsePanel.vue` | 解析面板：模型下拉（实时探测+手填，留空=轻任务模型）+ **「检测模型」门禁**（2026-09-10：点了且 `POST /llm/test` 成功才启用「开始解析」，避免先花钱再发现 key 不可用）+ 只读回显当前全局配置 + 2s 轮询进度与耗时 + 失败原因展示（卸载清理定时器）。 |
| `ModuleResultEditor.vue` | 结构化结果校对：分块折叠 + 就地编辑（保存走父组件传入的 `saveHandler`，只提交被改的那一块）。列定义驱动，六个列表区块共用一套渲染。 |
| `ModuleSelectPanel.vue` | KP 台模组骨架面板：只列已就绪模组，挂载/解绑 + 当前模组回显（广播驱动，不做本地乐观更新）；「管理模组库」带 `from_room` 跳转（同属房间工作区，来回不拆连接）。 |
| `CardReviewPanel.vue` | KP 台检卡面板（2026-09-10）：AI 逐卡列出问题（建议/明显不合理两级）+ 理由 + 改法，仅建议不拦截开团；有问题的卡排前面。 |
| `useQuitRoom.ts` | 退出确认：KP 且 waiting → 提示解散房间；统一 exitRoom 回首页。 |
| `useRoomReturn.ts` | 模组库的返回语义（2026-09-10）：按 `from_room` query 决定「返回 KP 控制台 / 返回大厅」；离开房间工作区时收尾拆连接（路由工作区名单见 `router/index.ts::ROOM_SCOPE_ROUTE_NAMES`）。 |
...
| `llm_usage` | 单行 token 全局总账：calls/prompt_tokens/completion_tokens/**cached_tokens**（命中前缀缓存的输入，prompt 的子集） |
| `room_usage` | 单房间 token 消耗（2026-09-10）：room_id PK + 同上四列；建房→解散一个场次，房间销毁时一并删除 |
...
> 迁移约定：早期改表结构=删库重跑 `python -X utf8 scripts/init_db.py --force`；4.4 起出现增量脚本（`scripts/migrate_44.py` 补 room.style_id、`scripts/migrate_45.py` 补 room.module_id + 建 module_scenario、`scripts/migrate_46.py` 补 llm_usage.cached_tokens、`scripts/migrate_47.py` 建 room_usage），后续结构变更优先写增量迁移。
