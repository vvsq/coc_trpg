...
| `app/api/` | REST 层：cards（卡/职业/技能/武器）、dice（掷骰）、rooms（房间）、kp（KP 操作）、suggestions（AI 模式/LLM/风格） |
| `app/ws/` | WebSocket：`manager.py` 连接/广播/定向；`rooms.py` 消息循环与心跳 |
| `app/agent/` | AI 主持核心：提示词组装、工具集、全自动主循环、协同建议、KP 风格、防剧透过滤 |
...
| `tests/` | pytest（17 个文件，180 用例），conftest 提供临时库+种子基建 |
...
| `POST /rooms/{id}/save`、`GET /rooms/{id}/saves`、`POST /rooms/{id}/load/{save_id}` | 存档/读档 |
...
**服务端→客户端信封 type（12 种）**：`chat_new` / `member_changed` / `roll_result` / `status_changed` / `scene_changed` / `room_state` / `room_dissolved` / `agent_mode_changed` / `kp_style_changed` / `suggestions` / `pong` / `error`。
...
- L3 模组骨架（临时方案，阶段 5 会做成模组库）：`_load_scenario_brief()` 读 `backend/data/scenario_brief.txt`（存在则整体注入，≤4000 字）。库里只放 `scenario_brief.example.txt` 模板，运行时文件不提交。
...
| `/room/:id/kp` | KPConsoleView | **KP 控制台**：三栏（成员面板/剧情流/KP 工具箱：开团·场景·检定暗骰·调 HP/SAN·AI 面板·风格·LLM·存读档） |
...
> 规划中但**尚未建**的表：goal §5.2 提过的 `chat_history`（历史实际存在 message 表）、`module_scenario`（模组库，阶段 5 做）。
> 迁移约定：早期改表结构=删库重跑 `python -X utf8 scripts/init_db.py --force`；4.4 起出现增量脚本 `scripts/migrate_44.py`（补 room.style_id 等），后续结构变更优先写增量迁移。
